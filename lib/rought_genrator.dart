...
final router = GoRouter(
  redirect: (state){
    String refernearn='';
    print("path redirect:${state.location.splitMapJoin(RegExp(r'/refer'),
        onMatch: (m) => '${m[0]}', // (or no onMatch at all)
        onNonMatch: (n) => '')}");
    if(state.location.splitMapJoin(RegExp(r'/refer'),
        onMatch: (m) => '${m[0]}', // (or no onMatch at all)
        onNonMatch: (n) => '') == '/refer'){
      return null;
    }
    ...
    switch(type){
      ...
      case NavigatoreTyp.pop:
        if(GoRouter.of(context).navigator!.canPop()) {
          ...
          context.go(/*"/${PrefUtils.prefs!.getString("branch")}*/ "/"+nUri(Routename.Home).path);
        }
        // context.vxNav.pop();
        // TODO: Handle this case.
        break;
