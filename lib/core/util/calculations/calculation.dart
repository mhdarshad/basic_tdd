class Calculations{
  ...
  // }
  int ip(dynamic val){
    return val != '' ? val is String?int.parse(val):int.parse((val??0).toString()) : 0;
  }
  double dp(dynamic val) {
    return (val != ''&& val !='null') ? val is String?double.parse(val):double.parse((val??0.0).toString()) : 0.0;
  }
